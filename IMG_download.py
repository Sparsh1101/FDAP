...
with rasterio.open('input.tif') as src:
    ...
    for i in range(num_x):
        for j in range(num_y):
            ...
            output_file = f"output_{i}_{j}.tif"
            with rasterio.open(output_file, 'w', driver='GTiff', width=256, height=256, count=src.count, dtype=src.dtypes[0], crs=src.crs, transform=src.window_transform(window)) as dst:

                # Write the data to the output TIF file
                dst.write(data)

# Output a list of the output file names
output_files = [f"output_{i}_{j}.tif" for i in range(
    num_x) for j in range(num_y)]
